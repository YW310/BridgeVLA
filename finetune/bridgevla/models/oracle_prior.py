...
class OraclePriorFusion(nn.Module):
    """Learn a zero-initialized residual correction to translation logits."""
    ...
    def __init__(self, hidden_channels: int = 16, multiscale: bool = False,
        ...
        self.net = nn.Sequential(
            nn.Conv2d(1 + prior_channels, hidden_channels, 3, padding=1),
            nn.GELU(),
    ...
    def _forward_relation(self, raw_logits, prior, instance_valid):
        ...
        if self.context is not None:
            ...
            if prior_channels == 2:
                context_parts.append((prior[:, :, 0] * prior[:, :, 1]).reshape(
                    batch_size * num_views, 1, height, width
                ))
            residual = residual + self.context(
                torch.cat(context_parts, dim=1)
            ).reshape(batch_size, num_views, height, width)
        relation_valid = _relation_valid_mask(instance_valid, batch_size)
    ...
    def forward(self, raw_logits, prior, instance_valid):
        ...
        if self.context is not None:
            ...
            residual = residual + self.context(
                torch.cat((features, interaction), dim=1)
            ).reshape(batch_size, num_views, height, width)
