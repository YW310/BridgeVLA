...
def test_standard_eval_signature_changes_with_config_and_runtime_setting(tmp_path):
    ...
    def signature(
            episode_length=50, bridgevla_aligned_objects=False,
        ...
        return build_eval_run_signature(
            ...
            use_input_place_with_mean=False,
            bridgevla_aligned_objects=bridgevla_aligned_objects,
            bridgevla_aligned_reference=bridgevla_aligned_reference)[1]

    original = signature()
    assert signature(episode_length=51) != original
    assert signature(bridgevla_aligned_objects=True) != original
    assert signature(bridgevla_aligned_reference=True) != original
