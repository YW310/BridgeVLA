...
4. 用零初始化的轻量 feature adapter 将 GT prior 注入共享视觉特征；
...
- 新增模式默认关闭；`rvt.oracle_prior_mode=none` 时不创建 adapter 参数，也不读取
...
- 第一、二阶段均先用 rank-16 feature adapter 将双通道实例 prior 注入 2048 维视觉特征；
  ...
  生成 gated FiLM。Stage 2 使用与局部 heatmap 相同的平移/缩放坐标系。Adapter 输出层
  零初始化，初始结果仍等于 baseline。
...
- `rlbench_o2_semantic_gt.yaml` 是正式 adapter-only 配置；旧
...
- 同一个 batch 内增加无梯度 baseline 动作支路，不重复 PaliGemma 前向；日志同时输出
  ...
  `trans_loss_base`/`trans_loss`，以及完整 T/R pair 的对应 `*_valid`
...
同一 renderer、同一三视图像素坐标
                 ↓
Oracle instance heatmap + frozen visual feature
                 ↓
zero-init relation-gated feature adapter
          ┌──────┴──────┐
          ↓             ↓
...
必须找到点云 preprocessing、三视图 renderer、共享视觉 feature、offline/online observation 入口。prior 必须在 action heads 前注入，不要在 3D action 生成后做最近点吸附。
...
oracle_prior/
  ...
  feature_adapter.py     # prior → shared visual feature residual
...
分别为 `P_T`、`P_R`。当前只训练低秩 feature adapter：
...
无效样本的 residual 必须强制为零。`trans` 直接来自 `L`，不再追加 logit residual。
...
同一 keyframe 分别解码 baseline 与 adapted logits，与专家下一 translation 比较。报告：
...
- adapter 零初始化时 adapted action 与 baseline 完全一致；
- adapter 输出层在第一个训练 step 能收到非零梯度；
- translation 解码位置与 R/G/C feature 采样位置一致；
...
4. 插入 zero-init feature adapter，验证初始 parity；
5. 只训练 adapter，再跑 offline paired evaluation；
6. 跑 online B0/O1/O2；
7. 扩展 O3-V/O3-F；
8. 最后加入隔离的 O4。
...
当前 O2 已采用 learned feature adapter，但训练和评估输入仍是 GT instance，因此仍属于
privileged Oracle 上界。验证信息有价值后，保持 prior/adapter 接口不变，把 provider
