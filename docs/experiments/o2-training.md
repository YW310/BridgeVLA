...
当前结构不再包含 translation logit 的 post-hoc fusion：
...
`trans_loss_raw` 和 `trans_loss_raw_valid` 已删除，因为删除 fusion 后它们与
`trans_loss` / `trans_loss_valid` 是同一张量，不再提供信息。
...
- 新 adapter-only O2 → 继续训练：使用 `--resume_checkpoint`。
- 旧 Adapter+Fusion checkpoint → 新结构：可用 `--init_checkpoint`；加载器只忽略已删除的
  fusion tensors，保留 adapter 权重。
- 旧 Adapter+Fusion checkpoint 不允许 `--resume_checkpoint`，因为 optimizer 参数组已经
  改变；代码会明确报错。

评估加载同样只过滤已废弃的 fusion keys，其他缺失或多余参数仍由 strict load 报错。
